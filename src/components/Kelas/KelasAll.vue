  ...
  <div class="kelas min-h-screen">
    <section class="content-kelas relative pt-28 md:pb-28 pb-16 px-2 md:px-0 h-auto">
      <div class="w-full text-center">
          <h1 class="font-popbold text-xl md:text-4xl">#MulaiAjaDulu</h1>
          <p class="font-popmed text-base md:text-lg md:mt-3 mt-2">Akses semua kelas berkualitas tinggi <br> dengan harga terjangkau di Mulai Belajar.</p>
      </div>
      <div class="relative">
        <div class="bg-nav-mobile sticky top-0 z-10 left-0">
          <div class="md:container mx-auto md:pt-16 pt-20 pb-3">
            <div class="grid grid-cols-1 md:grid-cols-2 mt-4 gap-3 md:w-2/5 w-full">
              <div>
                <p class="font-popmed text-xs mb-1">Pilih Paket</p>
              ...
              <div v-if="selectPaket == 0">
                <p class="font-popmed text-xs mb-1">Pilih Kelas</p>
          ...
          <div class="md:mt-8 mt-6 grid grid-cols-1 md:grid-cols-4 gap-5 md:gap-6">
            <div v-for="(item,i) in $store.state.kelas.managekelas" v-bind:key="i">
              <div class="shadow-xl border rounded-lg p-5">
                <h6 class="font-popmed text-xs text-blue-500">
                  {{item.kelas ? item.kelas.kelas == 0 ? '' + item.kelas.jenjang : item.kelas.kelas + ' ' + item.kelas.jenjang : ''}}</h6>
                <h3 class="font-popbold mt-1 text-base md:text-xl">{{item.pelajaran ? item.pelajaran.nama_pelajaran : ''}}</h3>
                <p class="font-popmed text-sm mt-2">{{item.pelajaran ? item.pelajaran.deskripsi : ''}}</p>
                <div class="mt-4">
                  <p class="font-popmed">Dengan belajar bersama Mulai Belajar, berarti kamu sudah turut membantu pendidikan untuk anak-anak lain di tempat tertinggal</p>
                </div>
                <div class="mt-4">
                  <p class="font-popbold text-base text-gray-500 line-through mb-1">{{item.harga_discount ? item.harga_discount : '' | currency('Rp. ')}}</p>
                  <div v-if="item.discount" class="px-4 mb-3 py-1 bg-red-500 text-white inline-flex rounded-lg">
                    <p class="font-popbold text-xs">Discount {{item.discount ? item.discount : '' | currency('Rp. ')}}</p>
                  </div>
                  <p class="font-popbold text-lg">{{item.harga_akhir ? item.harga_akhir : '' | currency('Rp. ')}}/ Bulan</p>
                </div>
                <div @click="checkoutParsial(item)" class="rounded-xl text-center md:mt-5 mt-4 py-2 w-full cursor-pointer hover:bg-yellow-500 bg-blue-500 text-white">
                    <p class="font-popbold text-sm">Daftar Sekarang</p></div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="md:mt-8 mt-6 grid grid-cols-1 md:grid-cols-4 gap-5 md:gap-6">
            <div v-for="(item,i) in $store.state.kelas.paket" v-bind:key="i">
              <div class="shadow-xl border rounded-lg p-5">
                <h6 class="font-popmed text-xs text-blue-500">
                  {{item.kelas ? item.kelas.kelas == 0 ? '' + item.kelas.jenjang : item.kelas.kelas + ' ' + item.kelas.jenjang : ''}}</h6>
                <h3 class="font-popbold mt-1 text-base md:text-xl">Kelas Intensif {{item.jurusan ? item.jurusan : ''}}</h3>
                <p class="font-popmed text-sm mt-1">{{item.deskripsi ? item.deskripsi : ''}}</p>
                <p class="mt-4 font-popmed">Semua focus mata pelajaran :</p>
                  ...
                  <div v-for="(item, ind) in item.detail_pelajaran" v-bind:key="ind">
                    <ol v-for="(items, inds) in item.pelajaran" v-bind:key="inds" class="list-inside">
                      <li class="inline-flex mt-1">
                        <p>{{ind + 1}}.</p>
                        <p class="font-popmed ml-2">{{items.pelajaran.nama_pelajaran}}</p>
                      </li>
                    </ol>
                  </div>
                </div>
                <div class="mt-4">
                  <p class="font-popbold text-base text-gray-500 line-through mb-1">
                  ...
                  <div class="px-4 mb-3 py-1 bg-red-500 text-white inline-flex rounded-lg">
                    <p class="font-popbold text-xs">Discount {{ (item.detail_pelajaran[0].pelajaran[0].harga_akhir + item.detail_pelajaran[1].pelajaran[0].harga_akhir 
                     + item.detail_pelajaran[2].pelajaran[0].harga_akhir) - item.harga | currency('Rp. ')}}</p>
                  </div>
                  <p class="font-popbold text-lg">{{item.harga ? item.harga : '' | currency('Rp. ')}}</p>
                </div>
